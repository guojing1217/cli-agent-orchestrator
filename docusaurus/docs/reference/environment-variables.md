...
| `CAO_CORS_ORIGINS` | Additional CORS origins (comma-separated) | |
...
| `CAO_SESSION_NAME` | Name of the parent CAO session |
...
CAO also supports a managed `.env` file at `~/.aws/cli-agent-orchestrator/.env`. Variables defined here are loaded and forwarded to agent sessions. Manage it with:
...
Variables in the `.env` file can be referenced in agent profiles and flow prompts using `${VAR}` syntax.
