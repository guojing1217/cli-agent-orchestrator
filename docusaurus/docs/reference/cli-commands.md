...
| `cao profile` | Manage agent profiles (list, show, validate, remove, templates, create) |
...
| Option | Description |
|--------|-------------|
| `--template NAME` | Template to base the profile on **(required)** |
...
Install agent profiles from a source.
