...
# 4. Manually trigger a flow (ignores schedule)
cao schedule run daily-standup

# 5. View the running session
...
A gating script decides at runtime whether the flow should execute. The script must output JSON with an `execute` boolean and an optional `output` object whose keys become template variables in the prompt.
...
When `execute` is `false`, the flow is skipped for that schedule tick. Template variables (`[[key]]`) in the prompt body are replaced with values from the `output` object.
...
# Manually run a flow (ignores schedule and gating script)
cao schedule run daily-standup
